...
Float_t Tracks::GetMaxPt()
{
  Float_t maxpt = 0;
  for(Int_t i=0; i<nTrk; i++)
  {
    if(!highPurity || fabs(trkEta[i])>2.0 || trkPtError[i]/trkPt[i]>0.05 || fabs(trkDz1[i]/trkDzError1[i])>3 || fabs(trkDxy1[i]/trkDxyError1[i])>3) continue; 
    if(trkPt[i]>maxpt) maxpt=trkPt[i];
  }
  return maxpt;
}

Float_t Tracks::GetMaxPtEta()
{
  Float_t maxptEta = 0;
  Float_t maxpt = 0;
  for(Int_t i=0; i<nTrk; i++)
...
void setupTrackTree(TTree *t,Tracks &tTracks,bool doCheck = 1)
{
   ...
   if (doCheck) {
      if (t->GetMaximum("nv")>54) { cout <<"FATAL ERROR: Arrary size of nv too small!!!  "<<t->GetMaximum("nv")<<endl; return;
 }      if (t->GetMaximum("nTrk")>maxEntryTrack) { cout <<"FATAL ERROR: Arrary size of nTrk too small!!!  "<<t->GetMaximum("nTrk")<<endl; return;
 }      if (t->GetMaximum("nParticle")>maxEntrySim) { cout <<"FATAL ERROR: Arrary size of nParticle too small!!!  "<<t->GetMaximum("nParticle")<<endl; return;
 }   }
}
