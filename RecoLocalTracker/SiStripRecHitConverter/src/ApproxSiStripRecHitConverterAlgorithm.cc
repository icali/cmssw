...
//unsure why this is causing issues
...
void ApproxSiStripRecHitConverterAlgorithm::
...
void ApproxSiStripRecHitConverterAlgorithm::
run(edm::Handle<edmNew::DetSetVector<SiStripApproximateClusterv2> > input, products& output) 
{ run(input, output, LocalVector(0.,0.,0.)); }



void ApproxSiStripRecHitConverterAlgorithm::
run(edm::Handle<edmNew::DetSetVector<SiStripApproximateClusterv2> > inputhandle, products& output, LocalVector trackdirection)
{
...
inline
bool ApproxSiStripRecHitConverterAlgorithm::
isMasked(const SiStripApproximateClusterv2 &cluster, bool bad128StripBlocks[6]) const 
{
  return false;
}
