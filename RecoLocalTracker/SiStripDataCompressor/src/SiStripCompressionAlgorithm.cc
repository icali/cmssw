...
void SiStripCompressionAlgorithm::commpressDetModule(const clusters_t& ncClusters, vcomp_clusters_t::TSFastFiller& compressedClusters) {
  ...
  //std::size_t toBeCompSize =0; //  
  //std::size_t suppportSize =0; //
  ...
  //  toBeCompSize+= itNcClusters->size();   //
  //  suppportSize +=4; //   
  }
  ...
  /*
  std::cout << "To be compressed zise: " << toBeCompSize 
    << " Compressed Amplitude Size: " << compAmplitudes.size() 
    << " Support Size: " << suppportSize <<std::endl;
  */  
